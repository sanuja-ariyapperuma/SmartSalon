...
namespace SalonCS.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            var publicKey = _asymmetricEncryption.GetPublicKey();

            if (String.IsNullOrEmpty(publicKey)) throw new Exception("Public key not found");

            var response = new ServiceResponse<string>
            {
                Data = Convert.ToBase64String(Encoding.UTF8.GetBytes(publicKey)),
                Success = true,
